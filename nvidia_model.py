...
points = (bins[:-1] + bins[1:]) * 0.5
plt.bar(points, hist, width=0.05)
plt.plot(
...
def load_img_steering(datadir, df):
    ...
    image_path_array = np.asarray(image_path)
...
X_train, X_val, y_train, y_val = train_test_split(
    image_paths, steerings, test_size=0.2, random_state=3
)
...
def nvidia_model():
    ...
    model.add(Conv2D(36, kernel_size=(5, 5), strides=2, activation="elu"))
    model.add(Conv2D(48, kernel_size=(5, 5), strides=2, activation="elu"))
